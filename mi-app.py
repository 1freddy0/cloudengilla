...
def recommend_oil():
    ...
    if not conditions:
        return jsonify({
            "recommendation": "select a driving or operating environment",
            "response": "No specific conditions selected. kindly click your driving or operating environment."
        })

    if len(conditions) >= 2:
        if set(conditions) == {"dusty", "heavy_traffic", "cold"}:
            return jsonify({
